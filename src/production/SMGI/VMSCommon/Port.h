...
static const char *SQL_GET_PORT
	= "SELECT port_id, port_num, misc_num, smg_type, slip_win, resp_time, resp_try, rpt_try,\
	  ...
	  login_name, login_pswd, corp_id, src_id, min_seq_id, max_seq_id, seq_id, is_iden FROM %s.port";
...
static const char *SQL_UPDATE_SEQ_ID
	= "UPDATE %s.port SET seq_id=%lu WHERE port_id=%d";
...
struct SSPInfo
{
	...
	char			_misc_num[MAX_ADDR_LEN+1];	//	管理平台接入号码
