...
** NT Service class
**
** following the command line switches
** -i: Install the service. 
**		After you've called your service with this switch, it should exist in 
**		Services Control Panel applet.
** -l: account
**		The service should run under the given account. This option is only 
**		useful with -i together. An account name must consist of a domain name
**		and a username separated by a backslash. A account name looks like:
**			alpha\wilbur or .\wilbur
** -p: password
**		The password for the account of the -l flag above.
** -u: Uninstall the service.
**		If the service is currently running, then stop it first.
** -s: Start the service.
**		The service must be installed before you can start it. Note that you can
**		not use -i and -s together.  
** -e: End a currently running service
** -d: Debug the service.
**		This starts the service as a consol application for easier debugging. You
**		can simulate a stop-request by pressing either ^C or ^Break
** -f: Start service as a faceless application(win95/98 mode).
**
*/
#pragma once

#ifdef NTSERVICES_EXPORTS
#define NTSERVICES_API __declspec(dllexport)
#else
#define NTSERVICES_API __declspec(dllimport)
#endif
...
namespace Process
{
	class NTSERVICES_API CNTService {
	private:
		static bool				m_fInstance;			// only one CNTService object per application
	...
	protected:
		...
		DWORD					m_dwErr;
		BOOL					m_fDebug;			// TRUE if -d was passed to the program
		...
		BOOL					m_fWinNT;			// TRUE, if this is running on WinNT FALSE on Win95
	...
	public:
		BOOL OsIsWin95() const { return ! m_fWinNT; }
		void SetupConsole();
	};
}

// Retrieve the one and only CNTService object:
NTSERVICES_API Process::CNTService * AfxGetService(void);